...
public class UI extends Canvas {
	...
	ConcurrentLinkedQueue<ColorationSommet> pointsToDraw = new ConcurrentLinkedQueue<ColorationSommet>();
	ConcurrentLinkedQueue<ColorationArc> segmentsToDraw = new ConcurrentLinkedQueue<ColorationArc>();
	...
	public void addPoint(double x, double y, int r, Color color) {
		...
		pointsToDraw.add(new ColorationSommet(x, y, r, color));
		repaint();
	}
	...
	public void addSegment(double x1, double y1, double x2, double y2, int f, Color color) {
		...
		segmentsToDraw.add(new ColorationArc(x1, y1, x2, y2, f, color));
		repaint();
	}

	// draw a point on the graphics    
	void drawPoint(Graphics2D g, ColorationSommet p) {
	...
	void drawSegment(Graphics2D g, ColorationArc s) {
		g.setColor(s.color);
	...
	public UI(String imageName, String title, double xmin, double xmax, double ymin, double ymax) {
		this.xmin = xmin;
		this.xmax = xmax;
		this.ymin = ymin;
		this.ymax = ymax;
	...
	public UI(String title, double xmin, double xmax, double ymin, double ymax, int size) {
		this.xmin = xmin;
		this.xmax = xmax;
		this.ymin = ymin;
		this.ymax = ymax;
